...
from model.Preprocess import PreprocessA, parse_age_to_midpoint
...
def _run_block_A(df_block: pd.DataFrame):
...
def run_parallel_features_A(trainA: pd.DataFrame, n_jobs: int):
    ...
    with parallel_backend('loky', inner_max_num_threads=1):
        results = Parallel(n_jobs=n_jobs, backend='loky', prefer='processes')(
            delayed(_run_block_A)(b) for b in blocks
        )
...
# ---------- 저장/병합 ----------
def concat_and_save_A(trainA: pd.DataFrame,
...
def main():
    ...
    t1 = time.time()
    A1, A2, A3, A4, A5, A6_7, A8_9_raw = run_parallel_features_A(trainA, n_jobs=args.jobs)
    ...
    # 저장
    t2 = time.time()
    concat_and_save_A(trainA, A1, A2, A3, A4, A5, A6_7, A8_9_feat,
