...
ctgan_param_dir = os.path.join(PARAM_DIR, "ctgan_synthesizer_A.pkl")
...
for item in syn_pos_list:
    ...
    print(f"ctgan_syn_A_pos_{item} data synthesizing...")
    t0 = time.time()
    synthetic_data = synthesizer.sample(num_rows=item)
    synthetic_data.to_csv(synthe_path, index=False)
    print(f"ctgan_syn_A_pos_{item} data synthesizing complete : {time.time()-t0:.3f}s")
