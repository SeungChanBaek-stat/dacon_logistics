...
def featurize_A1_lite(
    ...
    """A1 간단 전처리(정말 최소 피처 4개). 대회에서 변수 예산이 극도로 작을 때 사용.

    반환 컬럼(4개):
      - acc_overall: 유효반응률(=mean(A1_3))
      - mean_absRT_trim: RT 절댓값의 절단평균(10%) — 전반적 예측정확도(작을수록 좋음)
      - bias_direction_abs: mean_absRT_right - mean_absRT_left — 공간 편향(>0이면 오른쪽이 더 어렵다)
      - speed_sensitivity_abs: mean_absRT_fast - mean_absRT_slow — 속도 민감도(>0이면 빠를수록 오차↑)
    """
    if cfg is None:
        cfg = A1Config()
    ...
    for _, row in df.iterrows():
        ...
        a1_3 = _parse_commaseq(row.get(col_a1_3))  # response
        a1_4 = _parse_commaseq(row.get(col_a1_4))  # RT (편차)
        ...
        # 핵심만 계산: 유효 trial 마스크(간단 버전)
        rt_abs = np.abs(a1_4)
        valid_rt = _valid_rt_mask(rt_abs, cfg)
        valid = valid_rt & (a1_3 == 1) & ~np.isnan(a1_4)

        # acc_overall
        acc_overall = float(np.nanmean(a1_3)) if n_trials>0 else np.nan

        # mean_absRT_trim (valid만)
        v = rt_abs[valid]
        mean_absRT_trim = _trimmed_mean(v, cfg.trimmed_prop) if v.size>0 else np.nan

        # 방향별 평균(abs)
        def _mean_abs(mask):
            idx = np.where(mask & valid)[0]
            return float(np.mean(rt_abs[idx])) if idx.size>0 else np.nan

        mean_abs_left  = _mean_abs(a1_1 == cfg.dir_left)
        mean_abs_right = _mean_abs(a1_1 == cfg.dir_right)
        bias_direction_abs = (mean_abs_right - mean_abs_left) if (not np.isnan(mean_abs_right) and not np.isnan(mean_abs_left)) else np.nan

        # 속도별 평균(abs)
        mean_abs_slow   = _mean_abs(a1_2 == cfg.sp_slow)
        mean_abs_fast   = _mean_abs(a1_2 == cfg.sp_fast)
        speed_sensitivity_abs = (mean_abs_fast - mean_abs_slow) if (not np.isnan(mean_abs_fast) and not np.isnan(mean_abs_slow)) else np.nan

        rows.append({
            "acc_overall": acc_overall,
            "mean_absRT_trim": mean_absRT_trim,
            "bias_direction_abs": bias_direction_abs,
            "speed_sensitivity_abs": speed_sensitivity_abs,
        })
...
if __name__ == "__main__":
    ...
    feats_lite = featurize_A1_lite(demo, cfg=cfg)
    pd.set_option('display.max_columns', None)
    print(feats_lite.head(1).T)