...
def run_parallel_features(trainB: pd.DataFrame, n_jobs: int):
...
# ---------- 저장/병합 ----------
def concat_and_save(trainB: pd.DataFrame,
...
def main():
    ...
    parser.add_argument("--no-scale", action="store_true", help="B9_10 표준화 생략")
    ...
    t1 = time.time()
    B1, B2, B3, B4, B5, B6, B7, B8, B9_10_raw = run_parallel_features(trainB, n_jobs=args.jobs)
    print(f"[TIME] features: {time.time()-t1:.3f}s")

    # A8_9 스케일링(옵션)
    if args.no_scale or B9_10_raw.empty:
        B9_10_feat = B9_10_raw.copy()
    else:
        cols = B9_10_raw.columns.tolist()
        scaler = StandardScaler()
        B9_10_feat = pd.DataFrame(scaler.fit_transform(B9_10_raw[cols]),
                                 columns=cols, index=B9_10_raw.index)

    # 저장
    t2 = time.time()
    concat_and_save(trainB, B1, B2, B3, B4, B5, B6, B7, B8, B9_10_feat,
